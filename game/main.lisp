...
(defun update-player! (game)
  ...
  (let* ((fighter (game-fighter game))
    ...
    (cond ((beam-shooting? beam)
           (when (and (or (damage-from-shot! game beam (game-blobs game))
                      ...
                      (not (game-shot-counted game)))
             (incf (game-shots-hit game))
             (setf (game-shot-counted game) t)))
          (t (setf (game-shot-counted game) nil)))
...
(defun draw-white-text (msg pos-x pos-y size)
  (raylib:draw-text msg (1+ pos-x) (1+ pos-y) size +black+)
  (raylib:draw-text msg pos-x pos-y size +white+))
...
(defun render-waiting (game)
  ...
  (let ((fc (game-frame game)))
    ...
    (when (< (mod fc +frame-rate+) 30)
      (draw-white-text (format nil "PRESS START") (- 0 37) 40 10))
...
the player is pressing."
  ...
  (cond ((or (raylib:is-key-down +key-space+)
         ...
         (raylib:draw-rectangle (- +world-max-x+ 5)
                                (- +world-max-y+ 8)
                                2 2 +red+)
         (draw-white-text "SHOOT" (- +world-max-x+ 35) (- +world-max-y+ 25) 10))
        ((or (raylib:is-key-down +key-enter+)
             (raylib:is-gamepad-button-down *gamepad* +gamepad-b+))
         (setf (game-anything-pressed? game) t)
         (raylib:draw-rectangle (- +world-max-x+ 9)
                                (- +world-max-y+ 5)
                                2 2 +red+)
         (draw-white-text "BOMB" (- +world-max-x+ 30) (- +world-max-y+ 25) 10))
        ((raylib:is-gamepad-button-down *gamepad* +gamepad-x+)
         (setf (game-anything-pressed? game) t)
         (raylib:draw-rectangle (- +world-max-x+ 9)
                                (- +world-max-y+ 11)
                                2 2 +red+)
         (draw-white-text "WARP" (- +world-max-x+ 30) (- +world-max-y+ 25) 10))
        ...
        ;;
        ;; 2025-06-25 Do I still need to do that?
  ...
  (multiple-value-bind (vert hori) (axes-yields)
    (when (or (raylib:is-key-down +key-up+)
              ...
              (and (neg? vert) (not (close-to-zero? vert))))
      (setf (game-anything-pressed? game) t)
    ...
    (when (or (raylib:is-key-down +key-down+)
              ...
              (and (pos? vert) (not (close-to-zero? vert))))
      (setf (game-anything-pressed? game) t)
      (raylib:draw-rectangle (- +world-max-x+ 25)
                             (- +world-max-y+ 6)
                             2 2 +red+))
    (when (or (raylib:is-key-down +key-left+)
              ...
              (and (neg? hori) (not (close-to-zero? hori))))
      (setf (game-anything-pressed? game) t)
    ...
    (when (or (raylib:is-key-down +key-right+)
              ...
              (and (pos? hori) (not (close-to-zero? hori))))
      (setf (game-anything-pressed? game) t)
...
(defun render-playing (game)
  ...
  (let* ((fc (game-frame game))
    ...
    (when (and (< diff (* 1.5 +frame-rate+))
      ...
      (draw-white-text "REACH GOD MODE!" -47 0 10))
...
(defun render-dead (game)
  ...
  (render-enemies game)
  (draw-hud game)
  (draw-white-text (format nil "Accuracy: ~a%"
                           (round (* 100 (/ (game-shots-hit game)
                                            (max 0.1 (game-shots-made game))))))
                   -38 60 10)
